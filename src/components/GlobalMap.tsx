    ...
    <div className="container mx-auto px-4">
      ...
      >
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          Join doctors from over 150 countries collaborating across borders and specialties
        </p>
        <motion.div 
        ...
        >
          {["#GlobalHealth", "#MedicalNetwork", "#HealthcareCollaboration"].map((tag, index) => (
            <motion.span 
              key={tag}
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-xl overflow-hidden shadow-lg border border-gray-100 p-4">
          <div className="relative w-full aspect-[2/1] bg-gray-50 rounded-lg overflow-hidden">
            <svg viewBox="0 0 2000 1001" className="absolute inset-0 w-full h-full opacity-25 pointer-events-none z-0" aria-hidden="true">
              <g>
                <path
              ...
              {medicalHubs.map((hub, index) => {
                ...
                return connections.map((target, i) => {
                  ...
                  return (
                    <motion.path
                      key={`${index}-${i}`}
                      d={`M${x1},${y1} Q${(x1 + x2) / 2},${Math.min(y1, y2) - 120} ${x2},${y2}`}
                      stroke="#3B82F6"
                      strokeWidth="4"
                      fill="none"
            ...
            {medicalHubs.map((hub, index) => (
              <motion.div
                key={index}
                className={`absolute z-20 ${hub.size === 4 ? 'w-7 h-7' : hub.size === 3 ? 'w-5 h-5' : 'w-4 h-4'} rounded-full bg-[#3B82F6] flex items-center justify-center shadow-lg`}
                style={{
                  top: hub.y,
                  left: hub.x,
            ...
            <div className="absolute inset-0 flex items-center justify-center z-30">
              <motion.div 
                className="bg-white/80 rounded-lg p-6 backdrop-blur-sm shadow-md max-w-sm"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
